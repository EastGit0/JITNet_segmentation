...
class DetectronStuff(BaseDataSet):
    ...
    def _set_files(self):
        if self.split in ['train', 'val']:
            file_list = sorted(glob(os.path.join(self.root, 'frames', self.split + '/*.jpg')))
    ...
    def _load_data(self, index):
        ...
        image_path = os.path.join(self.root, 'frames', self.split, image_id + '.jpg')
        label_path = os.path.join(self.root, 'labels/colored_masks', self.split, image_id + '.png')
...
class DETECTRON(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1, partition = 'DetectronStuff',
        ...
        if partition == 'DetectronStuff': self.dataset = DetectronStuff(**kwargs)
        else: raise ValueError(f"Please choose DetectronStuff for DETECTRON")

        super(DETECTRON, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
